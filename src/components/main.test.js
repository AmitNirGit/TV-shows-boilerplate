...
describe(projectName, () => {
  ...
  describe("Home page test", () => {
    test("The app should have home page with all the popular tv shows", async () => {
      ...
      const firstShow = await page.$eval(
        getTvShowTitle(1),
        (show) => show.innerText
      );
      const secondShow = await page.$eval(
        getTvShowTitle(2),
        (show) => show.innerText
      );
      const lastShow = await page.$eval(
        getTvShowTitle(20),
        (show) => show.innerText
      );
  ...
  describe("Search test", () => {
    test("The app should have a working search bar and if the input submited empty the app should show the popular tv showes", async () => {
      await page.goto("http://localhost:3000");
      ...
      let firstShow = await page.$eval(
        getTvShowTitle(1),
        (show) => show.innerText
      );
      let secondShow = await page.$eval(
        getTvShowTitle(2),
        (show) => show.innerText
      );
      let lastShow = await page.$eval(
        getTvShowTitle(3),
        (show) => show.innerText
      );
  ...
  describe("One show test", () => {
    ...
    test("The single tc show page should include all the requirments", async () => {
      ...
      await page.waitForSelector(".one-show-description");
      const { data } = await axios.get(
        `https://www.episodate.com/api/show-details?q=23455`
      );
      ...
      const firstGenre = await page.$eval(
        getGenrePath(1),
        (genre) => genre.innerText
      );
      const secondGenre = await page.$eval(
        getGenrePath(2),
        (genre) => genre.innerText
      );
      const lastGenre = await page.$eval(
        getGenrePath(3),
        (genre) => genre.innerText
      );
      ...
      const status = await page.$eval(
        ".status",
        (showStatus) => showStatus.innerText
      );
